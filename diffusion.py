...
def denoise_with_mu(denoise_model, x_t, t, list_alpha, DATA_SIZE, device):

    alpha_t = list_alpha[t]
    beta_t = 1 - alpha_t
    mu_theta = denoise_model(x_t, t)
    x_t_before = torch.distributions.MultivariateNormal(loc=mu_theta, covariance_matrix=torch.diag(
